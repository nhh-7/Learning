...
- **Go 1.22 之前**：在循环中，迭代变量（如 `for i, v := range` 中的 `v`）是**复用**的。也就是说，整个循环过程中 `v` 的内存地址不变，只是值在变。如果你在循环里启动协程引用了 `&v`，所有协程最后都会拿到同一个值（通常是最后一个元素）。
- **Go 1.22 及之后**：修复了这个问题。每次循环迭代都会创建一个**新的变量**，地址是不同的，不再共享。

###  字符串拼接哪个最快？

- **推荐**：`strings.Builder` 或 `strings.Join`。
  - **原理**：`strings.Builder` 底层直接利用 `[]byte` 缓冲区，且在转为 string 时避免了内存拷贝（利用了 `unsafe` 转换），所以极快。
- **一般**：`bytes.Buffer`（比 Builder 稍慢，因为最后转 string 时有一次拷贝）。
- **最慢**：`+` 号拼接。因为它每次拼接都会创建一个新的字符串对象，对原来的字符串遍历放入到该对象中，频繁分配内存。
- **慢但方便**：`fmt.Sprintf`（涉及反射有性能损耗和复杂的格式化逻辑）。
...
Go语言的多返回值是通过在函数调用栈帧上预留空间并进行**值复制**来实现的。在函数调用发生时，Go编译器会计算出函数所有返回值的总大小。在为该函数创建栈帧时，就会在调用方（caller）的栈帧上，为这些返回值预留出连续的内存空间。
...
2. 扩容逻辑
   ...
   - 等量扩容：当溢出桶过多时，会进行整理，让数据更紧凑。
     - 当 B < 15 时，溢出桶数量 >= 2^B；当 B >= 15 时，溢出桶数量 >= 2^15。

### Map遍历为什么是无序的？

Go 的 `map` 底层是哈希表（Hash Table）。造成其遍历无序有以下几个技术原因：

- **哈希分布（Hashing）**： 当你向 `map` 插入键值对时，Key 会经过哈希函数计算得到一个哈希值，并根据这个值决定放入哪个 **Bucket（桶）**。由于哈希函数的离散性，Key 在桶中的分布本身就是乱序的，不取决于插入顺序。
...
最规范的做法是将 Map 的键（Key）取出来放入一个切片（Slice）中，用 `sort` 包对切片进行排序，然后根据这个有序的切片去遍历 Map。
...
无法对 map 的 key 或 value 进行取址。会发生编译报错，这样设计主要是因为map一旦发生扩容，key和 value 的位置就会改变，之前保存的地址也就失效了。

### Map 中删除一个key，它的内存会释放么？

**不会**。删除 key 只是把对应的 slot 标记为空（tophash 置为 empty），但bucket数组的空间不会被释放，只有重建 Map 才能真正释放。
...
1. 环形缓冲区：有缓冲channel内部维护一个固定大小的环形队列，用 buf 指针指向缓冲区，sendx 和recvx 分别记录发送和接收的位置索引。这样设计能高效利用内存，避免数据搬移。
2. 两个等待队列 sendq和recvq：用来管理阻塞的goroutine。 sendq 存储因channel满而阻塞的发送者，recvq 存储因channel空而阻塞的接收者。这些队列用双向链表实现，当条件满足时会唤醒对应的goroutine.
3. 互斥锁：hchan 内部有个mutex，所有的发送、接收操作都需要先获取锁，用来保证并发安全。虽然看起来可能影响性能，但Go的调度器做了优化，大多数情况下锁竞争并不激烈。

### 向channel发送数据的过程是怎样的？

1. **检查是否关闭**：如果**已关闭，Panic**。
2. **检查接收队列**：如果`recvq`队列不为空，说明有goroutine在等待接收数据，这时会直接把数据传递给等待的接收者，跳过缓冲区，这是最高效的路径。同时会唤醒对应的goroutine继续执行。
...
为了保证公平性，`select` 在执行前会通过一个随机算法**打乱所有 case 的轮询顺序**。防止某些 case 因为排在前面而被频繁触发（饥饿问题）。如果多个 Channel 同时准备好了，`select` 会随机选择一个执行。
...
如果没有任何 case 准备好，且没有 `default` 分支，当前的 Goroutine 就会**被阻塞**：

调度器会将当前的 Goroutine（G）包装成**多个 `sudog`**。将这些 `sudog` 分别放入 **`select` 中涉及到的所有 Channel 的等待队列**（发送队列或接收队列）中。调用 `gopark` 挂起协程，等待被唤醒。
...
信号量的实现其实跟mutex差不多，实现起来也很方便，主要通过信号量计数来保证。channel是Go最推崇的方式，它通过通信来传递数据所有权，从根源上避免竞争，更适合复杂的业务逻辑；而原子操作则针对最简单的**整型或指针**等进行无锁操作，性能最高，常用于实现计数器或状态位。
...
2. **饥饿模式**：当一个goroutine在等待队列中等待超过1毫秒（1ms）后，Mutex就会切换到此模式，讲究的是公平。在此模式下，锁的所有权会直接从解锁的goroutine移交给等待队列的头部，新来的goroutine不会自旋，必须排到队尾。这样可以确保队列中的等待者不会被"饿死"。
...
**通过“读写分离”和“空间换时间”来减少锁的竞争。**适用于**读多写少**（Read-Mostly）或者**多个协程写入不同 Key** 的场景

- **`read` (只读层)**：实际上是一个 `readOnly` 结构。它通过 `atomic` 包进行操作，意味着多个协程并发读取时**完全不需要加锁**。它包含了 `dirty` 中一部分数据的快照。
- **`dirty` (可写层)**：一个普通的 `map`。所有的新增写入操作都会先到这里，操作这里需要加 `mu` 互斥锁。
- **`entry`**：这是 `read` 和 `dirty` 共同指向的底层值对象。

1. 读取逻辑 (Load)
   1. **第一步**：先去 `read` 中查找。因为 `read` 是原子的，如果命中了，直接返回，全程无锁。
   2. **第二步**：如果 `read` 没中，加锁进入 `dirty` 中查找。
   3. **第三步（Miss 计数）**：如果在 `dirty` 中找到了，会将 `misses` 计数器加 1。
   4. **关键动作（提升）**：当 `misses` 的数量达到 `len(dirty)` 时，系统认为 `read` 的缓存效率太低，会将 `dirty` 直接提升为 `read`，然后清空旧的 `dirty`。

2. 写入逻辑 (Store)
   1. **更新已存在的 Key**：如果 `read` 中已经存在这个 Key，且没被标记为删除，尝试通过 `atomic` 直接更新 `entry` 的指针。这种情况也**不需要加锁**。
   2. **新增或更新 Dirty**：如果 `read` 中没有或者更新失败，加锁。如果 `dirty` 已经初始化，直接写 `dirty`。如果为nil，那么重新初始化dirty。
3. 删除逻辑 (Delete)
   1. **软删除**：如果在 `read` 中找到了 Key，它并不物理删除，而是利用 `atomic` 将 `entry` 的指针标记为 `nil`。
   2. **物理删除**：只有在 `dirty` 被提升为 `read` 的过程中，那些被标记为删除的 Key 才会被彻底过滤掉。
...
类型转换、类型断言本质都是把一个类型转换成另外一个类型。不同之处在于，类型断言是对接口变量进行的操作。对于**类型转换**而言，类型转换是在编译期确定的强制转换，转换前后的两个类型要相互兼容才行，语法是T(value)。而**类型断言**是运行期的动态检查，专门用于从接口类型中提取具体类型，语法是value.(T)
...
核心挑战是防止"对象消失"现象：**当黑色对象新增对白色对象的引用，同时灰色到白色的引用被删除时，白色对象可能被错误回收**。Go采用**混合写屏障**策略，在指针赋值时执行额外逻辑：**新建引用时将目标对象着为灰色，删除引用时将被删对象（被指向的对象）标为灰色**，这样确保关键对象不会丢失在标记过程中。

同时Go维护了**弱三色不变性**：允许黑色对象指向白色对象，但要保证从白色对象出发存在全灰色路径可达根对象。栈操作因为频繁且开销敏感，没有采用写屏障结束，而是做了特殊处理：标记开始和结束时分别扫描栈，中间过程不加写屏障。

### 什么是写屏障、混合写屏障，如何实现？

写屏障的本质是在编译器在指针赋值操作中插入的额外很短的指令，当执行`*slot = ptr`这样的指针赋值时，写屏障会**在赋值前后执行特定逻辑来标记相关对象，防止并发标记过程中对象被错误回收**。
